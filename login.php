...
if (!empty($data->Username) && !empty($data->Password)) {
    ...
    // If user doesn't exist, insert the admin, superadmin, or staff user
    if ($checkStmt->rowCount() == 0) {
        // Determine which user to create based on the username
        if ($username === 'dreamteam' || $username === 'eshercafeadmin' || $username === 'eshercafestaff') {
            if ($username === 'dreamteam') {
                $role = 'superadmin';
                $predefinedPassword = 'dreamwin'; // The password for the superadmin
            } elseif ($username === 'eshercafeadmin') {
                $role = 'admin';
                $predefinedPassword = 'adminpos24'; // Password for the admin
            } else {
                $role = 'staff';
                $predefinedPassword = 'kioskstaff2024'; // Password for the staff
            }

            // Hash the password using password_hash
            $hashedPassword = password_hash($predefinedPassword, PASSWORD_BCRYPT);
